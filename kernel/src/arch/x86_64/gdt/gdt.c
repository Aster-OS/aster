...
#define GDT_DESCRIPTOR_TYPE_CODE (1 << 7 | 1 << 4 | 1 << 3 | 1 << 1 | 1 << 0) // present, code/data, executable, readable, accessed
#define GDT_DESCRIPTOR_TYPE_DATA (1 << 7 | 1 << 4 | 1 << 1 | 1 << 0) // present, code/data, writable, accessed
#define GDT_DESCRIPTOR_TYPE_TSS_AVL (1 << 7 | 1 << 3 | 1 << 0 ) // present, TSS
#define GDT_DESCRIPTOR_TYPE_DPL(dpl) (dpl << 5)

#define GDT_DESCRIPTOR_FLAG_LONG_MODE (1 << 5)
...
static void gdt_set_tss_descriptor(struct tss_t *tss_ptr) {
    // we place the TSS descriptor after the segment descriptors
    ...
    tss_descriptor->type = GDT_DESCRIPTOR_TYPE_TSS_AVL;
...
void gdt_init(void) {
    ...
    // 0x08 - kernel code
    gdt_add_seg_descriptor(GDT_DESCRIPTOR_TYPE_CODE | GDT_DESCRIPTOR_TYPE_DPL(0), GDT_DESCRIPTOR_FLAG_LONG_MODE);

    // 0x10 - kernel data
    gdt_add_seg_descriptor(GDT_DESCRIPTOR_TYPE_DATA | GDT_DESCRIPTOR_TYPE_DPL(0), GDT_DESCRIPTOR_FLAG_LONG_MODE);

    // 0x18 - user code
    gdt_add_seg_descriptor(GDT_DESCRIPTOR_TYPE_CODE | GDT_DESCRIPTOR_TYPE_DPL(3), GDT_DESCRIPTOR_FLAG_LONG_MODE);

    // 0x20 - user data
    gdt_add_seg_descriptor(GDT_DESCRIPTOR_TYPE_DATA | GDT_DESCRIPTOR_TYPE_DPL(3), GDT_DESCRIPTOR_FLAG_LONG_MODE);

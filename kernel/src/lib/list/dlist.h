...
#define DLIST_LOCK_IRQSAVE(list)                                               \
    do {                                                                       \
        spin_lock_irqsave(&(list).lock);                                       \
    } while (0)

#define DLIST_UNLOCK_IRQRESTORE(list)                                          \
    do {                                                                       \
...
#define DLIST_INIT_SYNCED(list)                                                \
    do {                                                                       \
        (list).head = NULL;                                                    \
        (list).lock = SPINLOCK_INIT;                                           \
    } while (0)

#define DLIST_INSERT(list, node, links)                                        \
    do {                                                                       \
        if ((list).head == NULL) {                                             \
            (list).head = (node);                                              \
...
#define DLIST_INSERT_SYNCED(list, node, links)                                 \
    do {                                                                       \
...
#define DLIST_DELETE(list, node, links)                                        \
    do {                                                                       \
        if ((node)->links.prev) {                                              \
...
#define DLIST_DELETE_SYNCED(list, node, links)                                 \
    do {                                                                       \
        DLIST_LOCK_IRQSAVE(list);                                              \

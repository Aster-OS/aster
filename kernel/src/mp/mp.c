...
static inline void init_cpu_data(struct cpu_t *cpu, uint64_t id,
                                 uint64_t acpi_id, uint64_t lapic_id) {
    cpu->id = id;
    cpu->acpi_id = acpi_id;
...
void mp_init(struct limine_mp_response *mp) {
    ...
    while (__atomic_load_n(&initialized_cpu_count, __ATOMIC_SEQ_CST) !=
           mp->cpu_count) {
        pause();
    }

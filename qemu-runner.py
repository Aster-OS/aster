...
BASE_DIR = Path(__file__).parent.resolve() / "qemu-logs"


def get_log_file_path(subdirectory, filename):
    path = BASE_DIR / subdirectory
...
def main():
    ...
    parser.add_argument(
        "--debug", action="store_true", help="run QEMU for debugging with GDB"
    )
    parser.add_argument(
        "--debugcon",
        action="store",
        choices=["file", "stdio"],
        help="enable QEMU debugcon",
    )
    ...
    parser.add_argument(
        "--log", action="store", choices=["file", "stderr"], help="enable QEMU logs"
    )
    parser.add_argument(
        "--monitor", action="store_true", help="enable QEMU monitor (stdio only)"
    )
    parser.add_argument(
        "--nographics", action="store_true", help="disable QEMU graphics"
    )
    parser.add_argument(
        "--serial",
        action="store",
        choices=["file", "stdio"],
        help="enable QEMU serial logging",
    )
    parser.add_argument(
        "--smp", action="store", default="1", help="number of CPUs", metavar="COUNT"
    )
    parser.add_argument(
        "--verbose", action="store_true", help="print the QEMU command line"
    )
    ...
    if args.log:
        qemu_flags += ["-d", "int", "-M", "smm=off", "-no-reboot", "-no-shutdown"]
